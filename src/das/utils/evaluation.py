...
def evaluate_clustering(label, pred, calc_acc=False, total_probs=None):
    label_np = label.detach().cpu().numpy()
    pred_np = pred.detach().cpu().numpy()

    nmi = metrics.normalized_mutual_info_score(label_np, pred_np)
    ami = metrics.adjusted_mutual_info_score(label_np, pred_np)
    ari = metrics.adjusted_rand_score(label_np, pred_np)
    f = metrics.fowlkes_mallows_score(label_np, pred_np)
    if not calc_acc:
        return nmi, ami, ari, f, -1
    # pred_adjusted = get_y_preds(label, pred, len(set(label)))
    # acc = metrics.accuracy_score(pred_adjusted, label)
    if total_probs is not None:
        acc, match, reordered_preds = hungarian_evaluate(
            label,
            pred,
            total_probs,
        )
        return (
            ...
            acc,
            match,
            reordered_preds.cpu().detach().numpy(),
        )
    else:
        acc, match, reordered_preds = hungarian_evaluate(label, pred, total_probs)
...
def hungarian_evaluate(
    ...
    confusion_matrix_file="confusion.pdf",
    percent=[1.0],
):
    ...
    match = _hungarian_match(
        predictions, targets, preds_k=num_classes, targets_k=num_classes
    )
    # np.save("imagenet_match.npy", np.array(match))
    reordered_preds = torch.zeros(num_elems, dtype=predictions.dtype).to(
        predictions.device
    )
    ...
    print(
        "Using {} Samples to Estimate Pseudo2Real Label Mapping, Acc:{:.4f}".format(
            int(num_elems), acc
        )
    )
    ...
    #     print(
    #         "Using {} Samples to Estimate Pseudo2Real Label Mapping, Acc Top-5 :{:.4f}".format(
    #             int(num_elems), top5
    #         )
    #     )
