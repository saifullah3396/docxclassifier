...
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/JB2CyAwGqYxdG5W/download"
CHECKPOINT_PATH="${BASE_PATH}/Tobacco3482_docxclassifier_fpn_base.pt"
...
# first evaluate models without the FPN. The original models were trained using CV2 resizing instead of PIL resizing. This causes huge differences in perfs so we keep it the same
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/HjHCFoYkkbaPR7o/download"
CHECKPOINT_PATH="${BASE_PATH}/Tobacco3482_docxclassifier_fpn_large.pt"
$SCRIPT_DIR/../../../../scripts/evaluate.sh \
    +evaluate/document_classification=with_aug_and_preprocess \
    base/data_args=document_classification/tobacco3482 \
    dataset_config_name=default \
...
# first evaluate models without the FPN. The original models were trained using CV2 resizing instead of PIL resizing. This causes huge differences in perfs so we keep it the same
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/R3y8e5HNrAXPMto/download"
CHECKPOINT_PATH="${BASE_PATH}/Tobacco3482_docxclassifier_fpn_xlarge.pt"
$SCRIPT_DIR/../../../../scripts/evaluate.sh \
    +evaluate/document_classification=with_aug_and_preprocess \
    base/data_args=document_classification/tobacco3482 \
    dataset_config_name=default \
