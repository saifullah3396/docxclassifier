...
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/spHfzK6Ezxc5MPb/download"
CHECKPOINT_PATH="${BASE_PATH}/RvlCdip_docxclassifier_base.pt"
...
# first evaluate models without the FPN. The original models were trained using CV2 resizing instead of PIL resizing. This causes huge differences in perfs so we keep it the same
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/bWNHCjobFo3YYAK/download"
CHECKPOINT_PATH="${BASE_PATH}/RvlCdip_docxclassifier_large.pt"
$SCRIPT_DIR/../../../../scripts/evaluate.sh \
    +evaluate/document_classification=with_aug_and_preprocess \
    base/data_args=document_classification/rvlcdip \
    dataset_config_name=default_test \
...
# first evaluate models without the FPN. The original models were trained using CV2 resizing instead of PIL resizing. This causes huge differences in perfs so we keep it the same
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/ABBH35X7sJGXSyF/download"
CHECKPOINT_PATH="${BASE_PATH}/RvlCdip_docxclassifier_xlarge.pt"
$SCRIPT_DIR/../../../../scripts/evaluate.sh \
    +evaluate/document_classification=with_aug_and_preprocess \
    base/data_args=document_classification/rvlcdip \
    dataset_config_name=default_test \
