...
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/5WAaiDpoZSBBbB7/download"
CHECKPOINT_PATH="${BASE_PATH}/RvlCdip_docxclassifier_fpn_base.pt"
...
# first evaluate models without the FPN. The original models were trained using CV2 resizing instead of PIL resizing. This causes huge differences in perfs so we keep it the same
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/HWoPYQPsMeA5XXK/download"
CHECKPOINT_PATH="${BASE_PATH}/RvlCdip_docxclassifier_fpn_large.pt"
$SCRIPT_DIR/../../../../scripts/evaluate.sh \
    +evaluate/document_classification=with_aug_and_preprocess \
    base/data_args=document_classification/rvlcdip \
    dataset_config_name=default_test \
...
# first evaluate models without the FPN. The original models were trained using CV2 resizing instead of PIL resizing. This causes huge differences in perfs so we keep it the same
BASE_PATH="https://cloud.dfki.de/owncloud/index.php/s/cRbpwQ3834HzMDQ/download"
CHECKPOINT_PATH="${BASE_PATH}/RvlCdip_docxclassifier_fpn_xlarge.pt"
$SCRIPT_DIR/../../../../scripts/evaluate.sh \
    +evaluate/document_classification=with_aug_and_preprocess \
    base/data_args=document_classification/rvlcdip \
    dataset_config_name=default_test \
